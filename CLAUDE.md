...
- **Tous les solides à chiffres passent par le même pipeline** (`buildDieByType` → `_chamferSolid` + `buildNumberedDie`) : faces planes, arêtes nettes, sommets légèrement chanfreinés, `flatShading`. Pas de corps lissé ni de « boule » : d48/d60/d120 sont les solides de Catalan (`catalanDie`).
...
- **d4** : numéroté aux sommets comme un vrai d4 (3 chiffres par face, valeur = sommet du haut). Pose d'arrêt = pointe vers le haut, **face vers le joueur** (chiffre du résultat droit), inclinaison `_D4_TILT`.
